...
@router.get("/get-inventory/by-mat-code/{code}", name="Fact-Mat-Inv:get-inventory-by-mat-code", response_model=Dict, status_code=status.HTTP_200_OK)
async def get_inventory_by_mat_code(
    code: str,
    page_number: int = 1,
    page_size: int = 10,
    order: str = "",
    ...
    if not is_authorized(current_user, "Fact-Mat-Inv:get-inventory-by-mat-code"):
        return handle_result(ServiceResult(AuthExceptions.AuthUnauthorizedException()))
    else:
        result = await FactoryRawMaterialInventoryService(db).get_inventory_by_mat_code(
            code,
            page_num=page_number,
            page_size=page_size,
            order=order,
...
@router.get("/get-inventory/by-fact-identifier/{identifier}", name="Fact-Mat-Inv:get-inventory-by-identifier", response_model=Dict, status_code=status.HTTP_200_OK)
async def get_inventory_by_fact_identifier(
    identifier: str,
    page_number: int = 1,
    page_size: int = 10,
    order: str = "",
    ...
    if not is_authorized(current_user, "Fact-Mat-Inv:get-inventory-by-identifier"):
        return handle_result(ServiceResult(AuthExceptions.AuthUnauthorizedException()))
    else:
        result = await FactoryRawMaterialInventoryService(db).get_inventory_by_fact_identifier(
            identifier,
            page_num=page_number,
            page_size=page_size,
            order=order,
